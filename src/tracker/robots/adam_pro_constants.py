"""Adam-Pro (23-DoF) robot constants and construction helpers."""
...
ADAM_PRO_XML: Path = get_asset_path("adam_pro", "adam_pro.xml")

# NOTE: As a first baseline, we assume Adam-Pro shares actuator baselines with Adam-SP.
# If/when Adam-Pro hardware parameters differ, update these constants accordingly.
...
ADAM_PRO_ACT_LEG_PITCH = BuiltinPositionActuatorCfg(
...
ADAM_PRO_ACT_LEG_ROLL = BuiltinPositionActuatorCfg(
...
ADAM_PRO_ACT_LEG_YAW = BuiltinPositionActuatorCfg(
...
ADAM_PRO_ACT_ANKLE_PITCH = BuiltinPositionActuatorCfg(
...
ADAM_PRO_ACT_ANKLE_ROLL = BuiltinPositionActuatorCfg(
...
ADAM_PRO_ACT_WAIST_ROLL_YAW = BuiltinPositionActuatorCfg(
...
ADAM_PRO_ACT_WAIST_PITCH = BuiltinPositionActuatorCfg(
...
ADAM_PRO_ACT_SHOULDER = BuiltinPositionActuatorCfg(
...
ADAM_PRO_ACT_ELBOW = BuiltinPositionActuatorCfg(
...
ADAM_PRO_ARTICULATION = EntityArticulationInfoCfg(
  actuators=(
    ADAM_PRO_ACT_LEG_PITCH,
    ADAM_PRO_ACT_LEG_ROLL,
    ADAM_PRO_ACT_LEG_YAW,
    ADAM_PRO_ACT_ANKLE_PITCH,
    ADAM_PRO_ACT_ANKLE_ROLL,
    ADAM_PRO_ACT_WAIST_ROLL_YAW,
    ADAM_PRO_ACT_WAIST_PITCH,
    ADAM_PRO_ACT_SHOULDER,
    ADAM_PRO_ACT_ELBOW,
  ),
  soft_joint_pos_limit_factor=0.9,
)

ADAM_PRO_INIT_STATE = EntityCfg.InitialStateCfg(
  pos=(0.0, 0.0, 0.90),
  rot=(0.0, 0.0, 0.0, 1.0),
  lin_vel=(0.0, 0.0, 0.0),
  ang_vel=(0.0, 0.0, 0.0),
...
ADAM_PRO_COLLISIONS = (
  CollisionCfg(
    ...
    contype={r".*_collision$": 1},
    conaffinity={r".*_collision$": 1},
    priority={r"^(toeLeft_collision|toeRight_collision)$": 1, r".*_collision$": 0},
    friction={r"^(toeLeft_collision|toeRight_collision)$": (0.6,)},
  ),
)

ADAM_PRO_KEYFRAMES: dict[str, EntityCfg.InitialStateCfg] = {"default": ADAM_PRO_INIT_STATE}


def get_assets(meshdir: str) -> dict[str, bytes]:
  assets: dict[str, bytes] = {}
  update_assets(assets, ADAM_PRO_XML.parent / "meshes", meshdir, recursive=True)
  return assets


def get_spec() -> mujoco.MjSpec:
  spec = mujoco.MjSpec.from_file(str(ADAM_PRO_XML))
  spec.assets = get_assets(spec.meshdir)
  for joint_name in ADAM_PRO_INIT_STATE.joint_pos:
    spec.joint(joint_name).damping = _baseline_joint_damping(joint_name)
  return spec


def get_adam_pro_robot_cfg() -> EntityCfg:
  """Return a fresh Adam-Pro (23-DoF) robot configuration instance."""
  return EntityCfg(
    init_state=ADAM_PRO_INIT_STATE,
    collisions=ADAM_PRO_COLLISIONS,
    spec_fn=get_spec,
    articulation=ADAM_PRO_ARTICULATION,
  )


ADAM_PRO_ACTION_SCALE: dict[str, float] = {}
for actuator in ADAM_PRO_ARTICULATION.actuators:
  assert isinstance(actuator, BuiltinPositionActuatorCfg)
  effort = actuator.effort_limit
  stiffness = actuator.stiffness
  assert effort is not None
  for expr in actuator.target_names_expr:
    ADAM_PRO_ACTION_SCALE[expr] = 0.25 * effort / stiffness
